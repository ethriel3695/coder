...
type screenBackend struct {
	...
	// partial matching.  Enforcing a UUID should guarantee we match on the right
	// session.
	id string
...
// be called once.
func (b *screenBackend) start(_ context.Context) error {
	// If we could, we would want to spawn the daemon here and attach each
	// connection to it but since doing that spawns the daemon with a hardcoded
	// 24x80 size it is not a very good user experience.  Instead we will let the
	// attach command spawn the daemon on its own which causes it to spawn with
	// the specified size.
	b.id = uuid.NewString()

	tempdir := filepath.Join(os.TempDir(), "coder-screen")
	if b.configFile == "" {
		b.configFile = filepath.Join(tempdir, "config")
	}
	if b.socketsDir == "" {
		b.socketsDir = filepath.Join(tempdir, "sockets")
	}
	...
	err := os.MkdirAll(b.socketsDir, 0o700)
	if err != nil {
		return err
	}
...
func (b *screenBackend) attach(ctx context.Context, _ string, conn net.Conn, height, width uint16) (pty.PTYCmd, error) {
	...
	cmd.Env = append(b.command.Env, "TERM=xterm-256color", "SCREENDIR="+b.socketsDir)
...
// considered a success state (for example "no session" when quitting).  The
// command will be retried until successful, the timeout is reached, or the
// context ends (in which case the context error is returned).
func (b *screenBackend) sendCommand(ctx context.Context, command string, successErrors []string) error {
	...
	run := func() (bool, error) {
		...
		//nolint:gosec
		cmd := exec.CommandContext(ctx, "screen", "-S", b.id, "-X", command)
		cmd.Env = append(os.Environ(), "SCREENDIR="+b.socketsDir)
		cmd.Stdout = &stdout
		err := cmd.Run()
