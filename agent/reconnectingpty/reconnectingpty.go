...
type Options struct {
	Logger slog.Logger
	// ScreenConfigFile is the location of the screen configuration file.
	ScreenConfigFile string
	// ScreenSocketsDir is the location of the directory where screen should put
	// its sockets.
	ScreenSocketsDir string
...
func New(ctx context.Context, cmd *pty.Cmd, options *Options) *ReconnectingPTY {
	...
	if options.BackendType == "" || options.BackendType == codersdk.ReconnectingPTYBackendTypeAuto {
		...
		options.Logger.Debug(ctx, "backend selection", slog.F("backend", options.BackendType))
	}
	...
	case codersdk.ReconnectingPTYBackendTypeScreen:
		...
		rpty.backend = &screenBackend{
			command:    cmd,
			configFile: rpty.options.ScreenConfigFile,
			logger:     rpty.options.Logger,
			metrics:    rpty.options.Metrics,
			socketsDir: rpty.options.ScreenSocketsDir,
		}
	default:
...
func (rpty *ReconnectingPTY) lifecycle(ctx context.Context) {
	...
	if err != nil {
		...
		err = xerrors.Errorf("reconnecting pty is done")
	}
	rpty.setState(StateDone, err)
}
